...
- 导航分组改为：
  ...
  - 用户治理：`/dashboard/tenants`、`/dashboard/memberships`、`/dashboard/audit-logs`
...
- 复用点 1：`100svh` 视口高度策略（参考 `apps/web/app/globals.css` 的用户控制台壳层）。
...
- 面包屑改为可点击导航（非当前项可点击）。
- 用户详情页支持显示实体名（优先用户名，降级短 ID）。
- IA 语义同步到面包屑（数据/用户治理层级）。
...
- 禁用语言切换能力（保留主题切换）。
- locale 统一为 `zh`。
- 清理用户详情页关键英文文案（标题、按钮、提示、确认弹窗、错误文案等）。
...
1. 侧边栏仅有三组：总览 / 数据 / 用户治理。
2. 数据组仅有：活跃度 / 用量 / 营收。
3. 用户治理组仅有：用户目录 / 成员权限 / 操作记录。
...
2. 若要加自动化 UI：
- 下一轮引入 Playwright 最小套件（登录、导航、用户治理三条冒烟流）。
