...
export default function TenantShareDialogTrigger({
  ...
  const cardSubtitle = useMemo(() => subtitle?.trim() || intentCopy.subtitle, [intentCopy.subtitle, subtitle]);
  ...
  }, [cardSubtitle, cardTitle, open, previewImageUrl, qrDataUrl]);
  ...
  return (
    <>
      {trigger({ onClick: handleOpen, pending: pending || posterPending })}
          ...
          >
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                ...
                <p id={titleId} className="mt-2 text-base font-semibold text-neutral-900 dark:text-neutral-100">
                  {cardTitle}
                </p>
                <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{intentCopy.body}</p>
              </div>
              <button
                type="button"
                aria-label="关闭分享弹窗"
            ...
            <div className="mt-3 rounded-2xl border border-black/5 bg-neutral-50 p-3 dark:border-white/10 dark:bg-neutral-950/50">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">扫码或复制链接分享</p>
                  <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{cardSubtitle}</p>
                </div>
              </div>
              ...
              <p className="mt-2 text-xs text-neutral-500 dark:text-neutral-400">{intentCopy.hint}</p>
            </div>
...
async function generateSharePoster(payload: SharePosterPayload): Promise<string> {
  ...
  ctx.fillText('Generated by Eggturtle Breeding Library', qrPanelX + 42, qrPanelY + qrPanelHeight - 28);

  return canvas.toDataURL('image/png');
}

function drawGlowCircle(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number, color: string) {
...
function drawHeroFallback(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number) {
