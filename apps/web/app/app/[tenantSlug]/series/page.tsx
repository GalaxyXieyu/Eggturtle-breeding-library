          ...
          {!loading && series.length > 0 ? (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {series.map((item) => (
                <article
                  key={item.id}
                  className="group overflow-hidden rounded-2xl border border-neutral-200/90 bg-white shadow-[0_12px_24px_rgba(15,23,42,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_16px_30px_rgba(15,23,42,0.12)]"
                >
                  <div className="relative">
                  ...
                  <button type="button" className="block w-full p-4 text-left" onClick={() => openEditor(item)}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate text-base font-semibold text-neutral-900">{item.code}</p>
                        <p className="truncate text-sm text-neutral-600">{item.name || '未命名系列'}</p>
                      </div>
                      <Badge variant={item.isActive ? 'success' : 'default'}>{item.isActive ? '启用' : '停用'}</Badge>
                    </div>
                    <p className="mt-2 line-clamp-2 min-h-10 text-sm leading-relaxed text-neutral-600">{item.description || '暂无描述'}</p>
